-- Migration pour synchroniser automatiquement child_subject_enrollments
-- avec les changements dans teacher_assignments
...
BEGIN
  ...
  -- ayant la même school_id et school_level, mais seulement si l'enrollment n'existe pas déjà
...
BEGIN
  ...
  IF NEW.deleted_at IS NOT NULL AND OLD.deleted_at IS NULL THEN
    ...
    DELETE FROM child_subject_enrollments
  ...
  IF NEW.deleted_at IS NULL AND OLD.deleted_at IS NOT NULL THEN
    -- Recréer les enrollments comme pour un INSERT
    INSERT INTO child_subject_enrollments (child_id, school_id, school_year_id, subject_id, selected)
    SELECT 
      c.id,
      NEW.school_id,
  ...
  IF NEW.subject_id != OLD.subject_id AND NEW.deleted_at IS NULL THEN
    -- Supprimer les enrollments de l'ancienne matière
    DELETE FROM child_subject_enrollments
    WHERE school_id = OLD.school_id
      AND school_level = OLD.school_level
      AND subject_id = OLD.subject_id
    ...
    -- Créer les enrollments pour la nouvelle matière
    INSERT INTO child_subject_enrollments (child_id, school_id, school_year_id, subject_id, selected)
    SELECT 
      c.id,
      NEW.school_id,
  ...
  IF (NEW.school_id != OLD.school_id OR NEW.school_level != OLD.school_level) AND NEW.deleted_at IS NULL THEN
    -- Supprimer les enrollments de l'ancienne combinaison école/niveau
    DELETE FROM child_subject_enrollments
    WHERE school_id = OLD.school_id
      AND school_level = OLD.school_level
      AND subject_id = OLD.subject_id
    ...
    -- Créer les enrollments pour la nouvelle combinaison école/niveau
    INSERT INTO child_subject_enrollments (child_id, school_id, school_year_id, subject_id, selected)
    SELECT 
      c.id,
      NEW.school_id,
...
DROP TRIGGER IF EXISTS trigger_sync_enrollments_on_assignment_insert ON teacher_assignments;
DROP TRIGGER IF EXISTS trigger_sync_enrollments_on_assignment_update ON teacher_assignments;

-- Créer les triggers
CREATE TRIGGER trigger_sync_enrollments_on_assignment_insert
AFTER INSERT ON teacher_assignments
FOR EACH ROW
WHEN (NEW.deleted_at IS NULL)
EXECUTE FUNCTION sync_enrollments_on_assignment_insert();

CREATE TRIGGER trigger_sync_enrollments_on_assignment_update
AFTER UPDATE ON teacher_assignments
FOR EACH ROW
EXECUTE FUNCTION sync_enrollments_on_assignment_update();

-- Fonction pour synchroniser les enrollments existants avec les affectations actuelles
-- (à exécuter manuellement une fois pour initialiser les enrollments)
...
BEGIN
  ...
  -- On doit joindre avec children pour obtenir le school_level
