...
export class AuthConfirmComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (accessToken) {
      ...
      await this.confirmWithTokenHash(tokenHash, type);
  ...
  private async confirmWithTokenHash(tokenHash: string, type: string): Promise<void> {

    const result = await this.confirmationService.confirmWithTokenHash(tokenHash, type);
