import { Injectable, inject, signal, Injector, Optional } from '@angular/core';
import { normalizeGameTypeName } from '../../utils/game-type.util';
import {
  ...
  GAME_TYPE_LIENS,
  GAME_TYPE_CASE_VIDE,
} from '../../utils/game-type.constants';
 ...
 * 
 * Note: Ce service utilise uniquement les fallbacks si SupabaseService n'est pas disponible
 * (par exemple dans l'admin où ENVIRONMENT n'est pas configuré)
 */
@Injectable({
  providedIn: 'root',
})
export class GameTypeStyleService {
  ...
  constructor() {
    // Essayer d'injecter SupabaseService de manière optionnelle
    // Cela évite les erreurs si ENVIRONMENT n'est pas disponible (ex: dans admin)
    try {
      // Utiliser une injection dynamique pour éviter les erreurs de compilation
      const injector = inject(Injector);
      // On ne peut pas injecter directement car le service nécessite ENVIRONMENT
      // On utilisera uniquement les fallbacks si Supabase n'est pas disponible
      this.supabaseClient = null;
    } catch (error) {
      // Si l'injection échoue, on utilisera uniquement les fallbacks
      this.supabaseClient = null;
    }
  }
   ...
   * Note: Cette méthode ne charge pas depuis la DB si SupabaseService n'est pas disponible
   * (par exemple dans l'admin). Dans ce cas, on utilisera uniquement les fallbacks.
   */
  private async loadGameTypesFromDB(): Promise<void> {
    // Si SupabaseClient n'est pas disponible, on ne charge pas depuis la DB
    // Le service utilisera uniquement les fallbacks
  ...
  private getFallbackStyle(gameTypeName: string): GameTypeStyle {
    ...
    const fallbackStyles: Record<string, GameTypeStyle> = {
      [normalizeGameTypeName(GAME_TYPE_QCM)]: { icon: '📝', colorCode: '#1976d2' },
      [normalizeGameTypeName(GAME_TYPE_MEMORY)]: { icon: '🧠', colorCode: '#7b1fa2' },
      [normalizeGameTypeName(GAME_TYPE_CHRONOLOGIE)]: { icon: '⏱️', colorCode: '#f57c00' },
      [normalizeGameTypeName(GAME_TYPE_SIMON)]: { icon: '🎮', colorCode: '#388e3c' },
      [normalizeGameTypeName(GAME_TYPE_IMAGE_INTERACTIVE)]: { icon: '🖼️', colorCode: '#c2185b' },
      [normalizeGameTypeName(GAME_TYPE_REPONSE_LIBRE)]: { icon: '✍️', colorCode: '#0288d1' },
      [normalizeGameTypeName(GAME_TYPE_VRAI_FAUX)]: { icon: '✓✗', colorCode: '#d32f2f' },
      [normalizeGameTypeName(GAME_TYPE_LIENS)]: { icon: '🔗', colorCode: '#5d4037' },
      [normalizeGameTypeName(GAME_TYPE_CASE_VIDE)]: { icon: '📋', colorCode: '#455a64' },
    };
