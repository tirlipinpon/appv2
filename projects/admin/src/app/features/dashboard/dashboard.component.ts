...
import { Application as ParentApplication } from '../parent/components/application/application';
import { Application as ChildApplication } from '../child/components/application/application';
import { Application as TeacherApplication } from '../teacher/components/application/application';
...
export class DashboardComponent implements OnInit {
  ...
  private readonly teacherApplication = inject(TeacherApplication);
  ...
  async ngOnInit() {
    ...
    if (this.activeRole) {
      // Pour le rôle parent, charger le profil et les enfants
      if (this.activeRole === 'parent') {
        this.parentApplication.loadParentProfile();
        this.childApplication.loadChildren();
        // Vérifier le statut après chargement (sera mis à jour automatiquement via les effects)
        this.parentStore.checkParentStatus();
      }
      
      // Pour le rôle prof, charger le profil professeur
      if (this.activeRole === 'prof') {
        this.teacherApplication.loadTeacherProfile();
        // Les affectations seront chargées automatiquement via l'effect loadAssignmentsEffect
      }
    }
  }
