...
export interface AideImageUploadResult {
  url: string;
  width: number;
  height: number;
  error: string | null;
...
export class AideMediaUploadService {
  private readonly supabaseService = inject(SupabaseService);
  private readonly bucketName = 'aides-images';
  private readonly maxFileSize = 10 * 1024 * 1024; // 10MB
  private readonly allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp', 'image/gif'];
  
  // Paramètres d'optimisation pour les images d'aide
  ...
  /**
   * Upload une image d'aide vers Supabase Storage
   * L'image sera automatiquement convertie en WebP et optimisée
   * @param file Le fichier image à uploader
   * @param gameId ID du jeu pour organiser les fichiers
   * @returns Observable avec l'URL de l'image et ses dimensions
   */
  uploadAideImage(file: File, gameId: string): Observable<AideImageUploadResult> {
    ...
    return from(this.optimizeImageToWebP(file)).pipe(
      switchMap((optimizedFile) => {
        ...
        const fileName = `${gameId}/${Date.now()}.webp`;
  ...
  /**
   * Copie une image d'aide d'un jeu vers un autre jeu
   ...
   */
  copyAideImageToGame(sourceImageUrl: string, targetGameId: string): Observable<AideImageUploadResult> {
    try {
      ...
      return from(
        ...
        switchMap((result) => {
          ...
          return this.uploadAideImage(file, targetGameId);
        }),
        catchError((error) => of({
          url: '',
          width: 0,
  ...
  /**
   * Supprime une image d'aide du storage
   */
  deleteAideImage(imageUrl: string): Observable<{ success: boolean; error: string | null }> {
    // Extraire le chemin du fichier depuis l'URL Supabase
    // Format: https://...supabase.co/storage/v1/object/public/aides-images/path/to/file.ext
