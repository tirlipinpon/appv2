...
import { ProfileSyncService, ChildStore, AuthService } from '../../../shared';
...
export const ParentStore = signalStore(
  ...
  withMethods((store, infrastructure = inject(Infrastructure), injector = inject(Injector)) => {
    // Obtenir ChildStore de manière lazy pour éviter les dépendances circulaires
    const getChildrenStore = (): any => {
      try {
        return injector.get(ChildStore, null);
      } catch {
        return null;
      }
    };
    ...
    checkParentStatus: rxMethod<void>(
      pipe(
        switchMap(() => {
          ...
          // Vérifier si on peut utiliser le cache ChildStore
